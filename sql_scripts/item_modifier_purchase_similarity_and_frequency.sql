...
select user_account,item_name,variant,num_variant_ordered,modification_type
...
from
(select user_account,item_name,variant,modification_type,count(distinct order_id) as num_variant_ordered from
(select user_account,item_name,order_id,case when num_mods=-1 then 'drop' when num_mods=0 then'kept_default' else 'add' end as modification_type,listagg(modifier_name,',') within group (order by modifier_name) variant
...
select a.*,b.top_variant_for_item
...
all_variants a
join
( select * from (select distinct user_account,item_name,variant as top_variant_for_item,modification_type,rank() over(partition by user_account,item_name order by RANDOM()) rownum
 from all_variants where rank_within_item=1) where rownum=1   ) b
on (a.user_account=b.user_account and a.item_name=b.item_name and a.modification_type=b.modification_type)
join
( select * from (select distinct user_account,variant as top_variant,modification_type,rank() over(partition by user_account order by RANDOM()) rownum
 from all_variants where overall_rank=1) where rownum=1   ) c
on (a.user_account=c.user_account and a.modification_type=c.modification_type)
