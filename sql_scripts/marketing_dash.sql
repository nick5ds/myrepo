...
,max(last_Activity_date) next_Activity_date
,max(post_campaign_activity_date)
...
left join mailchimp_blendo.blendo_campaigns c
 on (c.recipients_segment_opts_saved_segment_id =s.segment_id)
left join (
...
)  mrkt where segment_name like 'Ma%' and rank=1  
) post_email
