  ...
  (SELECT DISTINCT sm.*, 
   ...
   FROM   mailchimp_blendo.blendo_segment_members sm 
          JOIN mailchimp_blendo.blendo_segments s 
            ON( sm.segment_id = s.id ) 
          LEFT JOIN (SELECT COALESCE(b.id, c.campaign_id) 
                            AS 
                            campaign_id, 
          COALESCE(c.segment_id, recipients_segment_opts_conditions_value) AS 
                                                 segment_id, 
          COALESCE(b.settings_title, c.settings_title) 
                                       AS 
                                                 campaign_title, 
          email_address, 
          COALESCE(c.send_time, b.send_time) 
                                       AS 
                                                 campaign_send_time, 
          Max(CASE action 
